...
// 1. NEVER short-circuit on server-side Realtime configuration. The client
//    decides Realtime-vs-polling from NEXT_PUBLIC_SUPABASE_* INLINED AT BUILD
//    TIME; this route reads the Worker's RUNTIME env. Those two can disagree
//    (Cloudflare Workers/OpenNext ship runtime secrets, a CI-built artifact
//    has no build-time public vars), and when they did, polling clients got
//    the constant below: `lastVersion` froze on its first tick and no market
//    update ever reached the page. A client that calls this endpoint needs a
//    real fingerprint, so always compute one. Realtime clients never call it.
...
/**
 * Cheap market-state fingerprint for the polling fallback.
 * Realtime clients never call this; polling clients depend on it.
 */
